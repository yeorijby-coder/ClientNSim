...
public:
	...
	CButton m_radDest102;
	CButton m_radDest105;
	CButton m_radStoHs;
	...
	BOOL StoHsDataDelete(CString& pLuggNo, CString& pHsMcNo);
	BOOL ScDataDelete();
	BOOL JobMstDelete(CString pLuggNo);
	BOOL JobMstInsert(CString pHsMcNo, CString& pLuggNo, CString strDestPos);
	BOOL CvDataUpdate(CString pMcNo, CString pLuggNo, CString strDestPos);

	BOOL RetHsDataSelect(CString& strHS_MC_NO, CString& strSensor0DataRd);
	BOOL JobMstUpdate(CString pLuggNo);
};
