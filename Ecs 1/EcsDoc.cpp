...
BEGIN_MESSAGE_MAP(CEcsDoc, CDocument)
	...
	ON_COMMAND_RANGE(ID_MONITORING_CV, ID_MONITORING_PL, &CEcsDoc::OnCommandRangeMainFrameMONITORING)
...
CEcsDoc::~CEcsDoc()
{
	...
	if (m_pLogEqpErrSkinDlg != NULL) { delete m_pLogEqpErrSkinDlg; }
...
void CEcsDoc::OnCommandRangeMainFrameLOG(UINT nID)
{
	...
	case ID_LOG_IO:
	{
		if (m_pLogIoSkinDlg == NULL)
		{
			...
			this->m_pLogIoSkinDlg->Create(IDD_LOG_JOB);
			CRect MainRect;
			CRect Rect;
			CRect PosRect;
	...
	case ID_LOG_EQP_HIS:
	{
		if (m_pLogEqpErrSkinDlg == NULL)
		{
			m_pLogEqpErrSkinDlg = new CLogEqpErrHisSkinDlg(this);
			this->m_pLogEqpErrSkinDlg->Create(IDD_LOG_EQP_ERR_HIS);
			CRect MainRect;
			CRect Rect;
			CRect PosRect;
			::AfxGetApp()->GetMainWnd()->GetWindowRect(&MainRect);
			this->m_pLogEqpErrSkinDlg->GetWindowRect(&Rect);

			PosRect.left = ((MainRect.right - MainRect.left) - Rect.Width()) / 2;
			PosRect.top = ((MainRect.bottom - MainRect.top) - Rect.Height()) / 2;
			this->m_pLogEqpErrSkinDlg->SetWindowPos(&m_pLogEqpErrSkinDlg->wndTop, PosRect.left, PosRect.top,
				Rect.Width(), Rect.Height(),
				SWP_SHOWWINDOW);

		}
		::SetWindowPos(m_pLogEqpErrSkinDlg->m_hWnd, HWND_TOPMOST, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE);
		::SetWindowPos(m_pLogEqpErrSkinDlg->m_hWnd, HWND_NOTOPMOST, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE);
		ShowWindow(m_pLogEqpErrSkinDlg->m_hWnd, SW_SHOWNORMAL);
		break;
	}
	case ID_LOG_CLIENT:
	{
...
void CEcsDoc::OnCommandRangeMainFrameVIEW(UINT nID)
{
	...
	case ID_VIEW_JOBLIST:
	{
		if (m_pViewJobListDlg == NULL)
		{
			...
			this->m_pViewJobListDlg->Create(IDD_VIEW_JOBLIST);
	...
	case ID_VIEW_IFLIST:
	{
		if (m_pViewMesListDlg == NULL)
		{
			m_pViewMesListDlg = new CViewMesListDlg(this);
			this->m_pViewMesListDlg->Create(IDD_VIEW_IFLIST);

			CRect MainRect;
			CRect Rect;
			CRect PosRect;
			::AfxGetApp()->GetMainWnd()->GetWindowRect(&MainRect);
			this->m_pViewMesListDlg->GetWindowRect(&Rect);
			...
			PosRect.left = ((MainRect.right - MainRect.left) - Rect.Width()) / 2;
			PosRect.top = ((MainRect.bottom - MainRect.top) - Rect.Height()) / 2;
			this->m_pViewMesListDlg->SetWindowPos(&m_pViewMesListDlg->wndTop, PosRect.left, PosRect.top,
				Rect.Width(), Rect.Height(),
				SWP_SHOWWINDOW);

		}
		::SetWindowPos(m_pViewMesListDlg->m_hWnd, HWND_TOPMOST, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE);
		::SetWindowPos(m_pViewMesListDlg->m_hWnd, HWND_NOTOPMOST, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE);
		ShowWindow(m_pViewMesListDlg->m_hWnd, SW_SHOWNORMAL);
...
CSC_DATA* CEcsDoc::GetSC_DATA(CString strSC_NO)
{
	...
	for (POSITION pPosSc = m_MapEqps.GetStartPosition(); pPosSc != NULL; )
	{
		...
		pSc = (CSc*)pEqp;
		CSC_DATA* pSC_DATA = NULL;	// pSc->m_pInfo->m_MapSC_DATA[strSC_NO];
		BOOL bResult = pSc->m_pInfo->m_MapSC_DATA.Lookup(strSC_NO, pSC_DATA);
		if (pSC_DATA != NULL)
			return pSC_DATA;
	}
	return NULL;
...
BOOL CEcsDoc::GetQueryInsertClientLog(CString pWIN_ID, CString pLUGG_NO, CString pBARCODE, CString pMESSAGE)
{
	CString CRLF = _T("\r\n");
	CString strSql = _T("");
