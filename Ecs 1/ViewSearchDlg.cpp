...
void CViewSearchDlg::OnSetSearchCv()
{
	...
	if (strTrack == _T(""))
	{
		if (strSearchTyp == _T("0"))
		{
			AfxMessageBox(m_pDoc->GetMsgLangDef(_T("트랙번호가 없습니다")));
		}
		else if (strSearchTyp == _T("1"))
		{
			AfxMessageBox(m_pDoc->GetMsgLangDef(_T("작업번호가 없습니다")));
		}
		else if(strSearchTyp == _T("2"))
		{
			AfxMessageBox(m_pDoc->GetMsgLangDef(_T("적재용기(파렛트)가 없습니다")));
		}
		else if(strSearchTyp == _T("3"))
		{
			AfxMessageBox(m_pDoc->GetMsgLangDef(_T("적재용기(P-BOX)가 없습니다")));
		}
		return;
	}
	...
	CString strLOG_MSG = _T("SEARCH TEXT : ") + strTrack;
	m_pDoc->GetQueryInsertClientLog(_T("CViewSearchDlg"), strLOG_LUGG_NO, strLOG_BOTTOM_TRAY, strLOG_MSG);
}
