...
void CLogClientSkinDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_LBL_LOG_CLIENT_BCR_BOTTOM,					 m_lblClientLogBcrBottom);
	DDX_Control(pDX, IDC_LBL_LOG_CLIENT_MESSAGE,					 m_lblClientLogMessage);
	...
	DDX_Control(pDX, IDC_EDT_LOG_CLIENT_MESSAGE,					 m_edtClientLogMessage);
...
void CLogClientSkinDlg::RelocationControls()
{
	...
	ScreenToClient(&rc2);
	m_btnClientLogSearch.MoveWindow(rc2.left, rc2.top, sizeLarge.cx, sizeLarge.cy);
...
void CLogClientSkinDlg::RedrawImage()
{
	...
	m_btnClientLogSearch.SetBitmaps(Global.GetBitmap(IDX_BMP_BTN_BASE_LARGE), Global.GetRGB(IDX_RGB_MASK), 0, 0);
...
void CLogClientSkinDlg::RenameResource( EN_LANG enLang)
{
	...
	SetDlgItemText(IDC_LBL_LOG_CLIENT_MESSAGE, strValue);
	...
	SetDlgItemText(IDC_LBL_LOG_CLIENT_BCR_BOTTOM, strValue);
...
CString CLogClientSkinDlg::GetQrySelect_Main(int nRowCheck,BOOL bSearch)
{
	...
	CString CRLF = _T("\r\n");
	CString strMessage;
	CString strBCR_BOTTOM, strLUGG_NO, strREQUEST_ID, strEQP_NO = _T("");
	CString strUserIp;
	...
	m_edtClientLogBcrBottom.GetWindowText(strBCR_BOTTOM);
	m_edtClientLogUserIp.GetWindowText(strUserIp);
	//m_edtClientLogUserId.GetWindowText(strUserId);
	m_edtClientLogMessage.GetWindowText(strMessage);
	...
	if (strBCR_BOTTOM != _T(""))
	{
		strSql += CRLF + _T("    AND BOTTOM_TRAY LIKE '%") + strBCR_BOTTOM + _T("%'");
	}
	if(strUserIp != _T(""))
	{
	...
	if(strMessage != _T(""))
	{
		strSql += CRLF + _T("    AND MESSAGE LIKE '%") + strMessage + _T("%'");
	}
#if ORACLE
