...
void CLogWcsLogPgr::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_CBX_WCS_LOG_EQ_KIND, m_cbxWcsLogEqKind);
...
BOOL CLogWcsLogPgr::OnInitDialog()
{
	...
	CLib::BindCombo(m_cbxWcsLogEqKind, _T("EQP_TYP"), m_pDoc, (int)pEn, TRUE);
...
void CLogWcsLogPgr::RelocationControls()
{
	...
	ScreenToClient(&rc2);
	m_btnWcsLogSearch.MoveWindow(rc2.left - 10, rc2.top, sizeLarge.cx, sizeWideLarge.cy);
}
...
void CLogWcsLogPgr::RedrawImage()
{
	...
	strAppPath = strAppPath.Left(strAppPath.ReverseFind('\\')) + _T("\\rc_resource\\dlg_wcslog\\20x20\\");
	strAppPath2 = strAppPath2.Left(strAppPath2.ReverseFind('\\')) + _T("\\rc_resource\\dlg_wcslog\\128x32\\");

	CString strExtension = _T(".png");
	m_btnWcsLogSearch.SetBitmaps(Global.GetBitmap(IDX_BMP_BTN_BASE_LARGE), Global.GetRGB(IDX_RGB_MASK), 0, 0);
...
void CLogWcsLogPgr::RenameResource(EN_LANG enLang)
{
	TCHAR chrFileName[500];
	GetModuleFileName(NULL, chrFileName, MAX_PATH);
	CString strAppPath = _T("");
	...
	CString strFullPath = Global.GetConcatPath(strAppPath.Left(strAppPath.ReverseFind('\\')) + _T("\\rc_resource\\dlg_wcslog\\"), _T("dlg_wcslog"), strExtension);
	CString strValue = CLib::GetIniStringFromPath(strFullPath, _T("dlgname"), (int)enLang);
	if (strValue.IsEmpty())
		strValue = _T("ECS 프로그램 로그");	// 리소스 ini 부재 시 기본 제목
	SetWindowText(strValue + _T(" [WCS_LOG_PGR]"));
	...
	SetDlgItemText(IDC_GRP_WCS_SEARCH, strValue);

	strFullPath = Global.GetConcatPath(strAppPath.Left(strAppPath.ReverseFind('\\')) + _T("\\rc_resource\\dlg_wcslog\\"), _T("dlg_wcslog"), strExtension);
	strValue = CLib::GetIniStringFromPath(strFullPath, _T("whtyp"), (int)enLang);
	SetDlgItemText(IDC_LBL_WCS_LOG_WH_TYP, strValue);

	strFullPath = Global.GetConcatPath(strAppPath.Left(strAppPath.ReverseFind('\\')) + _T("\\rc_resource\\dlg_wcslog\\"), _T("dlg_wcslog"), strExtension);
	strValue = CLib::GetIniStringFromPath(strFullPath, _T("eqkind"), (int)enLang);
	SetDlgItemText(IDC_LBL_WCS_LOG_EQ_KIND, strValue);

	strFullPath = Global.GetConcatPath(strAppPath.Left(strAppPath.ReverseFind('\\')) + _T("\\rc_resource\\dlg_wcslog\\"), _T("dlg_wcslog"), strExtension);
	strValue = CLib::GetIniStringFromPath(strFullPath, _T("program"), (int)enLang);
	SetDlgItemText(IDC_LBL_WCS_LOG_PGR_NM, strValue);

	strFullPath = Global.GetConcatPath(strAppPath.Left(strAppPath.ReverseFind('\\')) + _T("\\rc_resource\\dlg_wcslog\\"), _T("dlg_wcslog"), strExtension);
	strValue = CLib::GetIniStringFromPath(strFullPath, _T("message"), (int)enLang);
	SetDlgItemText(IDC_LBL_WCS_LOG_MESSAGE, strValue);

	strFullPath = Global.GetConcatPath(strAppPath.Left(strAppPath.ReverseFind('\\')) + _T("\\rc_resource\\dlg_wcslog\\"), _T("dlg_wcslog"), strExtension);
	strValue = CLib::GetIniStringFromPath(strFullPath, _T("luggno"), (int)enLang);
	SetDlgItemText(IDC_LBL_WCS_LOG_LUGG_NO, strValue);

	strFullPath = Global.GetConcatPath(strAppPath.Left(strAppPath.ReverseFind('\\')) + _T("\\rc_resource\\dlg_wcslog\\"), _T("dlg_wcslog"), strExtension);
	strValue = CLib::GetIniStringFromPath(strFullPath, _T("result"), (int)enLang);
	SetDlgItemText(IDC_LBL_WCS_LOG_RESULT, strValue);

	strFullPath = Global.GetConcatPath(strAppPath.Left(strAppPath.ReverseFind('\\')) + _T("\\rc_resource\\dlg_wcslog\\"), _T("dlg_wcslog"), strExtension);
	strValue = CLib::GetIniStringFromPath(strFullPath, _T("insdatefr"), (int)enLang);
	SetDlgItemText(IDC_LBL_WCS_LOG_FROM_DATE, strValue);

	strFullPath = Global.GetConcatPath(strAppPath.Left(strAppPath.ReverseFind('\\')) + _T("\\rc_resource\\dlg_wcslog\\"), _T("dlg_wcslog"), strExtension);
	strValue = CLib::GetIniStringFromPath(strFullPath, _T("insdateto"), (int)enLang);
	SetDlgItemText(IDC_LBL_WCS_LOG_TO_DATE, strValue);
...
CString CLogWcsLogPgr::GetQrySelect_Main(int nRowCheck, BOOL bSearch)
{
	...
	CString strEqpType = m_cbxWcsLogEqKind.GetItemKey(m_cbxWcsLogEqKind.GetCurSel());
...
//	int nRowSum;
	...
	strSql += CRLF + _T("SELECT WLP.INS_DT AS INS_DT															");
	...
	if (strEqpType != _T("") && strEqpType != _T("ALL"))
	{
		strSql += CRLF + _T("    AND WLP.EQP_TYP =  ") + CLib::Quot(strEqpType);
	}
