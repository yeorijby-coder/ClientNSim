   ...
   cbx.ResetContent();
    ...
    strSql.Format(_T(" SELECT SHD.HS_NO	AS HS_NO																				 \n")
       ...
       _T("          WHERE SHD.WH_TYP = '%s'																					 \n")
        _T("            AND SHD.SC_NO = '%s'																					 \n")
		_T("            AND CASE WHEN '%s' = '1' THEN SHD.HS_NO IN ('02','04')													 \n")
		_T("            		 WHEN '%s' = '2' THEN SHD.HS_NO IN ('01','03','05','06','07','08','09','10','11')                \n")
		_T("            						  ELSE 1 = 1 END																 \n")
...
BOOL CLib::BindCombo_SC_NO_MANUAL(CComboBoxWrapper& cbx, CString strCDX_CD, CEcsDoc *pDoc, int nEN_LANG, CString strSC_GRP_NO, CString strSC_NO)
{
   ...
   strSql += CRLF + _T("   FROM SC_DATA_MURATA SD LEFT OUTER JOIN COMMON_CODE CCD ");
...
BOOL CLib::BindCombo_RTV_NO(CComboBoxWrapper& cbx, CEcsDoc *pDoc, int nEN_LANG)
{
   ...
   strSql = CRLF + _T("  SELECT RTV_NO, ") + pDoc->NVL + _T("(CCD.CCD_NM_KOR, '') AS RTV_NM");
...
BOOL CLib::BindCombo_RTV_JOB_TYP(CComboBoxWrapper& cbx, CEcsDoc* pDoc, int nEN_LANG)
{
    if (pDoc == NULL)                     return FALSE;
    if (pDoc->IsConnectDB() == FALSE) return FALSE;
    CString strSql;
    CString CRLF = _T("\r\n");
    CString strRTV_JOB_TYP;
    int nRowCnt = 0, j = 0;
    CString strMessage;
    cbx.ResetContent();

    strSql = CRLF + _T("  SELECT ") + pDoc->NVL + _T("(CCD.CCD_NM_KOR, '') AS RTV_JOB_TYP");
    strSql += CRLF + _T("   FROM RTV_DATA RD LEFT OUTER JOIN COMMON_CODE CCD ");
    strSql += CRLF + _T("                                ON CCD.CDX_CD = 'RTV_STEP' ");
    strSql += CRLF + _T("                               AND (CCD.CCD_CD = '1' OR CCD.CCD_CD = '3' OR CCD.CCD_CD = '10')");
    strSql += CRLF + _T("                               AND CCD.WH_TYP LIKE '%") + pDoc->m_WH_TYP + _T("%'");
    strSql += CRLF + _T("  WHERE RD.WH_TYP = ") + CLib::Quot(pDoc->m_WH_TYP);
    strSql += CRLF + _T("  ORDER BY RTV_JOB_TYP ");
    strSql = CLib::GetCommonCodeLang(strSql, (int)pDoc->m_enLang);
    ...
    for (int i = 0; i < nRowCnt; i++)
    {
        strRTV_JOB_TYP = pRsw->GetItem(_T("RTV_JOB_TYP"));
        cbx.AddString(strRTV_JOB_TYP);
        cbx.SetItemDataEx(i, strRTV_JOB_TYP);
