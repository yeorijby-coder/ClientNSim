...
void CLogIoSkinDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_CBX_IO_LOG_FROM_TRACK,			 m_cbxIOFromTrack);
	DDX_Control(pDX, IDC_CBX_IO_LOG_TO_TRACK,			 m_cbxIOToTrack);
...
CString CLogIoSkinDlg::GetQrySelect_Main(int nRowCheck, BOOL bSearch)
{
	...
	strTRACK_START = m_cbxIOFromTrack.GetItemKey(m_cbxIOFromTrack.GetCurSel());
	strTRACK_DEST = m_cbxIOToTrack.GetItemKey(m_cbxIOToTrack.GetCurSel());
	...
	strSql += CRLF + _T("	  ,") + m_pDoc->NVL + _T("(HIS.ITEM_INFO, ' ') AS ITEM_INFO			");
	strSql += CRLF + _T("	  ,") + m_pDoc->NVL + _T("(HIS.ITEM_INFO2, ' ') AS ITEM_INFO2		");
	strSql += CRLF + _T("	  ,") + m_pDoc->NVL + _T("(HIS.PAIR_LUGG_NO, ' ') AS PAIR_LUGG_NO	");
	...
	{
		strSql += CRLF + _T("	AND HIS.BCR_BOTTOM LIKE '%") + strTRAY_BOTTOM + _T("%'");
	}
	if (strLUGG_NO != _T(""))
	{
		strSql += CRLF + _T("	AND HIS.LUGG_NO LIKE '%") + strLUGG_NO + _T("%'");
	}
	if (strTRACK_START != _T("") && strTRACK_START != _T("ALL"))
	{
		strSql += CRLF + _T("	AND HIS.START_POS = '") + strTRACK_START + _T("'");
	}
	if (strTRACK_DEST != _T("") && strTRACK_DEST != _T("ALL"))
	{
