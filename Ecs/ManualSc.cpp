...
void CManualSc::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_GRP_SC_MANUAL_SERACH,		   m_grpScSearch);
	...
	DDX_Control(pDX, IDC_BTN_SC_MANUAL_CLEAR,		   m_btnScManualClear);
...
BEGIN_MESSAGE_MAP(CManualSc, CSkinDialog)
	...
	ON_BN_CLICKED(IDC_BTN_SC_MANUAL_CLEAR, &CManualSc::OnBnClickedBtnScManualClear)
...
void CManualSc::RelocationControls()
{
	...
	m_btnScManualClear.GetWindowRect(&rc2);
	ScreenToClient(&rc2);
	m_btnScManualClear.MoveWindow(rc2.left, rc2.top, sizeLarge.cx, sizeLarge.cy);
...
void CManualSc::RenameResource( EN_LANG m_enLang)
{
	...
	SetDlgItemText(IDC_BTN_SC_MANUAL_CLEAR, strValue);

	strFullPath = Global.GetConcatPath(strAppPath.Left(strAppPath.ReverseFind('\\')) + _T("\\rc_resource\\dlg_scmanual\\"), _T("dlg_scmanual"), strExtension);
	strValue = CLib::GetIniStringFromPath(strFullPath, _T("search"), (int)m_enLang);
	SetDlgItemText(IDC_GRP_SC_MANUAL_SERACH, strValue);
...
void CManualSc::RedrawImage()
{
	...
	m_btnScManualClear.SetBitmaps(Global.GetBitmap(IDX_BMP_BTN_BASE_LARGE), Global.GetRGB(IDX_RGB_MASK), 0, 0);
	m_btnScManualClear.SetIcon(Global.HICONFromPATH(Global.GetConcatPath(strAppPath, _T("cut"), strExtension)), NULL, 5, 5);
	m_btnScManualClear.GetWindowRect(&rc);
	ScreenToClient(&rc);
	m_btnScManualClear.MoveWindow(rc.left, rc.top, szLarge.cx, szLarge.cy);
}
...
void CManualSc::OnBnClickedBtnScManualSave()
{
	...
	if (SelectScStatus() == FALSE)
	{ AfxMessageBox(m_pDoc->GetMsgLangDef(_T("작업중입니다"))) + _T(" ") + m_pDoc->GetMsgLangDef( _T("종료 혹은 삭제 후 진행하세요.")); return;}

	if (strJobTyp == _T("1"))
	{
		strStartLocBankFork1 = _T("00");
		strStartLocBayFork1 = _T("000");
		strStartLocLevFork1 = _T("00");
		strDestHsFokr1 = _T("0");
	}
	else if (strJobTyp == _T("2"))
	{
		strStartHsFork1 = _T("0");
		strDestLocBankFork1 = _T("00");
		strDestLocBayFork1 = _T("000");
		strDestLocLevFork1 = _T("00");
	}
	...
	if (!m_pDoc->GetQueryInsertClientLog(_T("CManualSc"), strLOG_LUGG_NO, strLOG_BARCODE, strLOG_MSG))
	{
		m_pDoc->RollbackTrans_DLG();
		return;
	}

	strSql = _T(" UPDATE SC_DATA_MURATA										 \n");
	strSql+= _T("    SET CMD_RQ_YN = 'Y'									 \n");
	strSql+= _T("    ,CMD_RQ_ID	   = 'MOD'									 \n");
	strSql+= _T("    ,JOB_TYP      = '") + strJobTyp + _T("'				 \n");
	strSql+= _T("    ,LUGG_NO      = '9999'									 \n");
	strSql+= _T("    ,START_BANK   = '") + strStartLocBankFork1 + _T("'		 \n");
	strSql+= _T("    ,START_BAY    = '") + strStartLocBayFork1 + _T("'		 \n");
	strSql+= _T("    ,START_LEVEL  = '") + strStartLocLevFork1 + _T("'		 \n");
	strSql+= _T("    ,START_HSPOS  = '") + strStartHsFork1 + _T("'			 \n");
	strSql+= _T("    ,DEST_BANK    = '") + strDestLocBankFork1 + _T("'		 \n");
	strSql+= _T("    ,DEST_BAY     = '") + strDestLocBayFork1 + _T("'		 \n");
	strSql+= _T("    ,DEST_LEVEL   = '") + strDestLocLevFork1 + _T("'		 \n");
	strSql+= _T("    ,DEST_HSPOS   = '") + strDestHsFokr1 + _T("'			 \n");
	...
	if(isSuccess == TRUE)
	{
		...
		AfxMessageBox(m_pDoc->GetMsgLangDef(_T("SC 작업 생성 성공")));
		return;
	}
	m_pDoc->RollbackTrans_DLG();
	AfxMessageBox(m_pDoc->GetMsgLangDef(_T("SC 작업 생성 실패")));
	return;
...
BOOL CManualSc::SelectScStatus()
{
	...
	strSql = _T("");
	strSql += _T("SELECT *											\n");
	strSql += _T("  FROM SC_DATA_MURATA								\n");
	strSql += _T(" WHERE WH_TYP = '") + m_pDoc->m_WH_TYP + _T("'	\n");
	strSql += _T("   AND MC_NO = '") + strScNo + _T("'				\n");
	strSql += _T("   AND ITN_LUGG = '0'								\n");
...
void CManualSc::OnBnClickedBtnScManualClear()
{
	DataClear();
}

void CManualSc::DataClear()
{
	m_cbxScManualScGrpNo.SetCurSel(0);
	m_cbxScManualJobTyp.SetCurSel(0);
	m_cbxStartPosFork1.SetCurSel(0);
	m_cbxStartPosFork1.EnableWindow(FALSE);
	m_cbxDestPosFork1.SetCurSel(0);
	m_cbxDestPosFork1.EnableWindow(FALSE);
	m_edtManualScStartPosFk1.SetWindowText(_T("01-001-01"));
	m_edtManualScStartPosFk1.EnableWindow(FALSE);
	m_edtManualScDestPosFk1.SetWindowText(_T("01-002-01"));
