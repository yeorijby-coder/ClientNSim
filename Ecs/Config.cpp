...
void CConfig::LoadConfigDATABASE()
{
	...
	//0:None, 1:Oracle, 2:PostgreSql, 3:MS_SQL, 4:MY_SQL
		...
		::GetPrivateProfileString(_T("DB_1"), _T("USERID"), _T("HUONS") ,  szTemp, _MAX_PATH, ECS_INI_FILE);
		m_strDATABASE_USERID.Format(_T("%s"), szTemp); 
		::GetPrivateProfileString(_T("DB_1"), _T("USERPASSWORD"), _T("HUONS") ,  szTemp, _MAX_PATH, ECS_INI_FILE);
...
void CConfig::InitializeConfigUSER()
{
	...
	m_clrUSER_COLOR_STN_STO = RGB(255,0,255);
	m_clrUSER_COLOR_STN_RET = SKY_BLUE;//RGB(255,128,128);
	m_clrUSER_COLOR_HS_STO	= RGB(255,140,0);
	m_clrUSER_COLOR_HS_RET	= RGB(0,128, 255);
	...
	m_clrUSER_COLOR_STO_SUSPEND = RED;	//
	m_clrUSER_COLOR_RET_SUSPEND = RED;		//
...
//	::WritePrivateProfileString(_T("USER"), _T("USER_COLOR_WTW"),    CConvert::ToString(//), ECS_INI_FILE);
