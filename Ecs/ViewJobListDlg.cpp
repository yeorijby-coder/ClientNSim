...
BEGIN_MESSAGE_MAP(CViewJobListDlg, CSkinDialog)
	...
	ON_WM_SIZE()
	ON_WM_CLOSE()
	ON_COMMAND_RANGE(ID_JOB_INSERT, ID_JOB_SEARCH3, &CViewJobListDlg::OnCommandRangeButtonEvent)
	...
	ON_WM_ERASEBKGND()
	ON_WM_SYSCOMMAND()
	ON_BN_CLICKED(IDC_CHK_AUTO_SEL, &CViewJobListDlg::OnBnClickedChkAutoSel)
	ON_WM_TIMER()
END_MESSAGE_MAP()
...
void CViewJobListDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, ID_JOB_SEARCH3, m_btnJobSearch3);
	...
	DDX_Control(pDX, IDC_VIEW_JOBLIST_EDIT_BCR_BOTTOM, m_edtBcrBottom);
...
void CViewJobListDlg::ClearInputData()
{
	...
	m_edtLuggNum.SetWindowText(strLuggNum);
	m_edtBcrBottom.SetWindowText(strBcrBottom);
...
#pragma region 스프레드 초기화 
	...
	m_SpreadSheet.AddColHead(_T("출발지"), 7);
	m_SpreadSheet.AddColHead(_T("출발위치"), 11);
	m_SpreadSheet.AddColHead(_T("도착지"), 7);
	...
	m_SpreadSheet.AddColHead(_T("PAIR_LUGG_NO"), 15);
	m_SpreadSheet.AddColHead(_T("등록시간"), 21);
	m_SpreadSheet.AddColHead(_T("수정시간"), 21);
...
void CViewJobListDlg::UpdateJob()
{
	...
	if (AfxMessageBox(m_pDoc->GetMsgLangDef(_T("작업상태 및 우선순위를 수정 하시겠습니까? [ 작업번호 : ") + strLUGG_NO + _T(" ]")), MB_YESNO) != IDYES)
		return;
	...
	if (!m_pDoc->GetQueryInsertClientLog(_T("CViewJobListDlg"), strLOG_LUGG_NO, _T(""), strLOG_MSG))
	{
	...
	strSql.Format(_T("UPDATE JOB_MST     ")
		...
		_T("	    , JOB_PRIORITY    = '%s'  ")
		_T("    WHERE WH_TYP     = '%s'  ")
		_T("      AND LUGG_NO    = '%s'  "), strJobStatus, strJobPriority, strWH_TYP, strLUGG_NO);
	...
	{
		m_pDoc->RollbackTrans_DLG();
		AfxMessageBox(m_pDoc->GetMsgLangDef(_T("작업상태 및 우선순위 변경 실패")));
		return;
	}
	m_pDoc->CommitTrans_DLG();
	AfxMessageBox(m_pDoc->GetMsgLangDef(_T("작업상태 및 우선순위 변경 성공")));
	InitializeSpread(TRUE);
}
...
void CViewJobListDlg::JobComplete(CString strJOB_STATUS)
{
	...
	CString strWH_TYP, strLUGG_NO, strSC_NUM, strJOB_TYP, strBCR_BOTTOM;
	...
	strWH_TYP = m_SpreadSheet.GetValueTXT(1, m_nActiveRow);
	strLUGG_NO = m_SpreadSheet.GetValueTXT(2, m_nActiveRow);
	strSC_NUM = m_SpreadSheet.GetValueTXT(5, m_nActiveRow);
	strBCR_BOTTOM = m_SpreadSheet.GetValueTXT(9, m_nActiveRow);
	...
	{
		strMsg = "CV도착 보고";
		if (AfxMessageBox(m_pDoc->GetMsgLangDef(strMsg + _T("를 하시겠습니까?  [ LUGG_NO : ") + strLUGG_NO + _T(" ]")), MB_YESNO) != IDYES)
			return;

		if (strJOB_TYP == _T("1") || strJOB_TYP == _T("4") || strJOB_TYP == _T("5"))
		{
			AfxMessageBox(m_pDoc->GetMsgLangDef(_T("해당 작업구분은 ") + strMsg + _T("를 할 수 없습니다.")));
			return;
		}
	}
	else
	...
	//	int nActiveRow;
	CString strSTART_POS, strSTART_LOCATION, strDEST_POS, strDEST_LOCATION;

	strSTART_POS = m_SpreadSheet.GetValueTXT(3, m_nActiveRow);
	strSTART_LOCATION = m_SpreadSheet.GetValueTXT(4, m_nActiveRow);
	strDEST_POS = m_SpreadSheet.GetValueTXT(5, m_nActiveRow);
	strDEST_LOCATION = m_SpreadSheet.GetValueTXT(6, m_nActiveRow);
	strJOB_STATUS = m_SpreadSheet.GetValueTXT(8, m_nActiveRow);
	...
	if (strLUGG_NO == _T("")) { strLUGG_NO = _T("0"); }
	CString strLOG_MSG = _T("");
	strLOG_MSG.Format(_T("작업삭제 : 출발지[%s], 출발위치[%s], 도착지[%s], 도착위치[%s], 작업구분[%s], 작업상태[%s], 팔렛트번호[%s]"), strSTART_POS, strSTART_LOCATION, strDEST_POS, strDEST_LOCATION, strJOB_TYP, strJOB_STATUS, strBCR_BOTTOM);

	if (!m_pDoc->GetQueryInsertClientLog(_T("CViewJobListDlg"), strLUGG_NO, strBCR_BOTTOM, strLOG_MSG))
	{
		m_pDoc->RollbackTrans_DLG();
		return;
	}
	...
	strSql.Format(_T("DELETE FROM JOB_MST				")
		_T("				WHERE WH_TYP = '%s'			")
		_T("				  AND LUGG_NO= '%s'		"), m_pDoc->m_WH_TYP, strLUGG_NO);

	BOOL isSuccess = m_pDoc->ExcuteQueryString_DLG(strSql);

	if (isSuccess == TRUE)
	{
		if (strJOB_STATUS == "29" && strSC_NUM == "901")
		{
			strJOB_STATUS = "19";
		}
		else if (strJOB_STATUS == "19")
			strJOB_STATUS = "09";
		...
		{
			m_pDoc->RollbackTrans_DLG();
			AfxMessageBox(m_pDoc->GetMsgLangDef(_T("IF_LUGG_STA >> IF_STATUS 변경 실패")));
			return;
		}

		m_pDoc->CommitTrans_DLG();
		AfxMessageBox(m_pDoc->GetMsgLangDef(strMsg + _T(" 성공")));
		InitializeSpread(TRUE);
		return;
	}

	m_pDoc->RollbackTrans_DLG();
	AfxMessageBox(m_pDoc->GetMsgLangDef(_T("작업번호 ") + strLUGG_NO + +_T(" 삭제 실패")));
...
void CViewJobListDlg::DeleteJob()
{
	variant_t val;
	//	int nActiveRow;
	CString strLUGG_NO, strSTART_POS, strSTART_LOCATION, strDEST_POS, strDEST_LOCATION, strJOB_TYP, strJOB_STATUS, strBCR_BOTTOM;
	CString strSql;
	...
	strDEST_POS = m_SpreadSheet.GetValueTXT(5, m_nActiveRow);
	strDEST_LOCATION = m_SpreadSheet.GetValueTXT(6, m_nActiveRow);
	strJOB_TYP = m_SpreadSheet.GetValueTXT(7, m_nActiveRow);
	strJOB_STATUS = m_SpreadSheet.GetValueTXT(8, m_nActiveRow);
	strBCR_BOTTOM = m_SpreadSheet.GetValueTXT(9, m_nActiveRow);
...
#pragma region 해당 작업번호가 공PLT 작업인지 체크하는 부분 주석
...
#pragma endregion
	...
	{
		m_pDoc->CommitTrans_DLG();
		AfxMessageBox(m_pDoc->GetMsgLangDef(_T("작업번호 ") + strLUGG_NO + +_T(" 삭제 성공")));
		InitializeSpread(TRUE);
		return;
	}
...
void CViewJobListDlg::RenameResource(EN_LANG m_enLang)
{
	...
	strFullPath = Global.GetConcatPath(strAppPath.Left(strAppPath.ReverseFind('\\')) + _T("\\rc_resource\\dlg_jobinfo\\"), _T("dlg_jobinfo"), strExtension);
	strValue = CLib::GetIniStringFromPath(strFullPath, _T("search3"), (int)m_enLang);
	SetDlgItemText(ID_JOB_SEARCH3, strValue);
...
void CViewJobListDlg::RedrawImage()
{
	...
	m_btnJobSearch3.SetBitmaps(Global.GetBitmap(IDX_BMP_BTN_BASE_LARGE), Global.GetRGB(IDX_RGB_MASK), 0, 0);
	m_btnJobSearch3.SetIcon(Global.HICONFromPATH(Global.GetConcatPath(strAppPath, _T("search3"), strExtension)), NULL, 5, 5);
	m_btnJobSearch3.GetWindowRect(&rc);
	ScreenToClient(&rc);
	m_btnJobSearch3.MoveWindow(rc.left, rc.top, szLarge.cx, szLarge.cy);
...
CString CViewJobListDlg::GetQrySelect_Main(BOOL bSearch)
{
	...
	strSql.Format(_T(" SELECT ") + m_pDoc->NVL + _T("(CCD_WH_TYP.CCD_NM_KOR, JM.WH_TYP) AS WH_TYP 		\n")
		...
		_T("		         ,") + m_pDoc->NVL + _T("(JM.ITEM_INFO, ' ') AS ITEM_INFO					\n")
		_T("		         ,") + m_pDoc->NVL + _T("(JM.ITEM_INFO2, ' ') AS ITEM_INFO2					\n")
		_T("		         ,") + m_pDoc->NVL + _T("(JM.PAIR_LUGG_NO, ' ') AS PAIR_LUGG_NO				\n")
	...
	if (strBCR_BOTTOM != "")
	{
		strSql += _T("AND JM.BCR_BOTTOM LIKE '%") + strBCR_BOTTOM + _T("%' \n");
	}
...
void CViewJobListDlg::GetItemID(CStringArray& strArr)
{
	...
	strArr.Add(CConvert::ToString(ID_JOB_SEARCH3));
	...
	strArr.Add(CConvert::ToString(IDC_VIEW_JOBLIST_EDIT_BCR_BOTTOM));
...
void CViewJobListDlg::CopyJob()
{
	variant_t val;
	CString strSql;
	CString strWH_TYP, strLUGG_NO, strJOB_TYP, strSTART_POS, strDEST_POS, strBCR_BOTTOM;
	...
	if (AfxMessageBox(m_pDoc->GetMsgLangDef(_T("해당 작업을 복사 하시겠습니까?  [ LUGG_NO : ") + strLUGG_NO + _T(" ]")), MB_YESNO) != IDYES)
		return;
	...
	pCopyJob->START_POS = strSTART_POS;
	pCopyJob->BCR_BOTTOM = strBCR_BOTTOM;
	pCopyJob->COPY_YN = true;

	AfxMessageBox(m_pDoc->GetMsgLangDef(_T("작업 ") + strLUGG_NO + +_T(" 복사 성공")));
	InitializeSpread(TRUE);
}


void CViewJobListDlg::OnBnClickedChkAutoSel()
{
