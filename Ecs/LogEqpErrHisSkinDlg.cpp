...
{
	m_pDoc->m_pLogEqpErrSkinDlg = NULL;
	this->DestroyWindow();
}
...
void CLogEqpErrHisSkinDlg::RelocationControls()
{
	...
	ScreenToClient(&rc2);
	m_btnSearch.MoveWindow(rc2.left - 10, rc2.top, sizeLarge.cx, sizeLarge.cy);
}
...
//	m_SpreadSheet.m_rectWnd = rectTemp;//CRect(0, 0, 0, 0);
	...
	m_SpreadSheet.AddColHead(_T("에러발생시간"), 21);
	m_SpreadSheet.AddColHead(_T("창고타입"), 9);
	m_SpreadSheet.AddColHead(_T("설비종류"), 9);
	m_SpreadSheet.AddColHead(_T("설비번호"), 9);
	m_SpreadSheet.AddColHead(_T("에러코드"), 9);
	m_SpreadSheet.AddColHead(_T("에러메시지"), 41);
	m_SpreadSheet.AddColHead(_T("작업번호"), 8);
	m_SpreadSheet.AddColHead(_T("비고"), 14);
...
void CLogEqpErrHisSkinDlg::RenameResource( EN_LANG enLang)
{
	...
	if (strValue.IsEmpty())
		strValue = _T("설비에러기록");	// 리소스 ini 부재 시 기본 제목
	SetWindowText(strValue + _T(" [EQP_ERR_HIS]"));
...
CString CLogEqpErrHisSkinDlg::GetQrySelect_Main(int nRowCheck,BOOL bSearch)
{
	...
	strSql = CRLF +  _T("SELECT EEH.ERROR_DT AS ERROR_DT											    ");
	strSql += CRLF + _T("      ,") + m_pDoc->NVL + _T("(CD_WH_TYP.CCD_NM_KOR, '10') AS WH_TYP			");
