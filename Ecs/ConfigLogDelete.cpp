...
//	m_SpreadSheet.m_rectWnd = rectTemp;//CRect(0, 0, 0, 0);
	...
	m_SpreadSheet.AddColHead(_T("컬럼명"), 23);
	m_SpreadSheet.AddColHead(_T("세팅값"), 8);
	m_SpreadSheet.AddColHead(_T("세팅단위"), 10);
...
void CConfigLogDelete::RelocationControls()
{
	...
	ScreenToClient(&rc2);
	m_btnConfigLogDeleteUpdate.MoveWindow(rc2.left, rc2.top, sizeLarge.cx, sizeLarge.cy);
...
void CConfigLogDelete::RedrawImage()
{
	...
	m_btnConfigLogDeleteUpdate.SetBitmaps(Global.GetBitmap(IDX_BMP_BTN_BASE_LARGE), Global.GetRGB(IDX_RGB_MASK), 0, 0);
...
//	int nRowSum;
	...
	strSql += CRLF + _T(" ORDER BY KEY_COLUMN desc																 ");
...
void CConfigLogDelete::OnBnClickedBtnConfigLogDeleteUpdate()
{
	...
	if (strTABLE_NAME_HIDE == _T("empty_ret_auto_del"))
	{
		if (strCYCLE != _T("Y") && strCYCLE != _T("N"))
		{
			AfxMessageBox(m_pDoc->GetMsgLangDef(_T("Y/N 값만 설정할 수 있습니다.")));
			return;
		}
	}
	else
	...
	if(AfxMessageBox(m_pDoc->GetMsgLangDef(_T("세팅값을 변경하시겠습니까? 컬럼명 : ")) + strTABLE_NAME, MB_YESNO) == IDNO) {	return;	}
	
	m_pDoc->BeginTrans_DLG();

	CString strLOG_LUGG_NO = _T("");
	CString strLOG_MSG = _T("");
	strLOG_MSG.Format(_T("세팅값 변경 : 컬럼명[%s], 세팅값[%s]"),strTABLE_NAME, strCYCLE) ;

	if (!m_pDoc->GetQueryInsertClientLog(_T("CConfigLogDelete"), strLOG_LUGG_NO, _T(""), strLOG_MSG))
	{
	...
	if(isSuccess == TRUE)
	{
		...
		AfxMessageBox(m_pDoc->GetMsgLangDef(_T("세팅값 변경 성공")));
		m_edtConfigLogDeleteDay.SetWindowText(_T(""));
