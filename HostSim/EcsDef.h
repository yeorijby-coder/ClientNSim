...
// @.크레인 대수. 이 현장은 901~911 로 11대다(SC_HS_DEF).
//   5 로 박혀 있어 6호기 이상은 상태전문을 버렸고(CHostSv::Parsing),
//   CalcurateLocation 도 1~5 만 훑어 로직그룹의 Scs 가 6 이상이면
//   쓸 수 있는 크레인을 하나도 못 찾아 입고 지시가 작업번호 0 으로 나갔다.
//   HostSimLogic.xml 의 <ScCount> 는 읽기만 하고 쓰지 않는다. 여기가 기준이다.
#define SC_CNT					11
// @.작업대 상태를 담는 배열 크기. 작업대 번호를 그대로 첨자로 쓰므로
//   현장의 가장 큰 번호보다 커야 한다. EcsDefine.xml 의 DestCode 는
//   101~251 이다(3층 BOX 피킹대 251 이 최대). 여유를 두어 260 으로 둔다.
//   7 로 박혀 있어 1층 101 부터 전부, 3층은 말할 것도 없이 버려졌다.
#define STO_STN_CNT				260
