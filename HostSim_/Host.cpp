	...
	case	CMD_COMPLETE:
		{
			...
			int nLuggNum = _ttoi(strFrame.Mid(3, 4));
			CString ucJobDefine	= strFrame.Mid(2, 1);
			int nStep = _ttoi(strFrame.Mid(8, 1));
	...
	case	CMD_STATUS:
		{
			...
			for (i = 0 ; i < SC_CNT ; i++)
			{
				int nScStatus = _ttoi(strFrame.Mid((i * 7) + 4, 1));
				m_pDoc->m_nScStatus[i] = nScStatus;
			}
			for (i = 0 ; i < STO_STN_CNT ; i++)
			{
				BOOL bStoStnStatus = _ttoi(strFrame.Mid((SC_CNT * 7) + 2 + i, 1));
				m_pDoc->m_bStoStation[i] = bStoStnStatus;
			}
...
BOOL CHostCl::CalcurateLocation(BOOL bSto)
{
...
// n2ndStn은 이동일때만 사용됨!
int CHostCl::JobOrder(int nJobType, int n1stStn, int n2ndStn, BOOL bManual)
{
	int nUserData = 0;
	CString strLog, strTempMsg, strStation;
	...
	case 3:		
		...
		//	return 0;
		m_pDoc->m_strRetLocation[nLuggNum] = m_pDoc->m_strLocation;
		...
		strTempMsg.Format(_T("%d%c%04d%d%-20s%03d%7s%03d%03d%7s%04d%03d%1s%-20s%-20s"),
			...
			_T("PalletID(20자리)"),		// 5	// Pallet Size
			0,							// 6	// Source Station
			m_pDoc->m_strLocation,		// 7	// Source Bank, Bay, Level
			0,							// 8	// Route Station
